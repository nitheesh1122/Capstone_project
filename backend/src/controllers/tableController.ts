...
export const addTable = async (req: Request, res: Response) => {
    ...
    try {
        const [result] = await pool.query<ResultSetHeader>(
            'INSERT INTO restaurant_tables (table_number, capacity, table_type, status) VALUES (?, ?, ?, ?)',
            [table_number, capacity, type || 'Regular', 'Available']
        );
        res.status(201).json({ message: 'Table added', id: result.insertId });
    } catch (error) {
        require('fs').writeFileSync('f:/Capstone_project-main/Capstone_project-main/backend/backend_error.log', JSON.stringify(error, Object.getOwnPropertyNames(error)));
...
export const updateTable = async (req: Request, res: Response) => {
    ...
    try {
        ...
        await pool.query(
            'UPDATE restaurant_tables SET status = COALESCE(?, status), capacity = COALESCE(?, capacity), table_type = COALESCE(?, table_type) WHERE id = ?',
