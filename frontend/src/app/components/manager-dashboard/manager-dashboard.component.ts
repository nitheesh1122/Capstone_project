                                                                                                                                                                    ...
                                                                                                                                                                    <div class="action-buttons">
                                                                                                                                                                        <button class="btn-action primary"> Add Items </button>
                                                                                                                                                                            <button class="btn-action secondary"> Checkout </button>
                                                                                                                                                                             <button class="btn-action secondary" (click)="updateTableStatus('Available')"> Make Available </button>
                                                                                                                                                                             <button class="btn-action primary" (click)="updateTableStatus('Occupied')"> Mark Occupied </button>
                                                                                                                                                                             <button class="btn-action secondary" (click)="updateTableStatus('Reserved')"> Mark Reserved </button>
                                                                                                                                                                                 </div>
                                                                                                                                                                                </div>
                                                                                                                                                                                </div>
...
export class ManagerDashboardComponent implements OnInit {
    ...
    fetchTables() {
        ...
        this.tableService.getTables().subscribe({
            next: (data) => {
                ...
                this.tables = data.map((t: any) => ({
                    id: t.id,
                    name: `Table ${t.table_number}`,
                    status: t.status,
                    seats: t.capacity,
                    shape: t.table_type || 'Square'
                }));
    ...
    updateTableStatus(status: string) {
        if (!this.selectedTable) return;

        const tableId = this.selectedTable.id;
        this.tableService.updateTable(tableId, { status }).subscribe({
            next: (updatedTable) => {
                // Update local state
