...
@Component({
    ...
    <div class="container">
      <div class="header">
        ...
        <div *ngIf="isManager()" class="manager-controls">
            <button mat-raised-button color="accent" (click)="toggleAddForm()">
                <mat-icon>add</mat-icon> {{ showAddForm ? 'Cancel' : 'Add Table' }}
            </button>
        </div>
      </div>
            ...
            <mat-card-content>
                <div class="form-row">
                    <mat-form-field appearance="outline">
                        ...
                        <input matInput [(ngModel)]="newTable.table_number" placeholder="e.g. T-10">
                    </mat-form-field>
                    <mat-form-field appearance="outline">
                        <mat-label>Capacity</mat-label>
                        <input matInput type="number" [(ngModel)]="newTable.capacity">
                    </mat-form-field>
                    <mat-form-field appearance="outline">
                        <mat-label>Shape</mat-label>
                        <select matNativeControl [(ngModel)]="newTable.type">
                            <option value="Square">Square</option>
                            <option value="Round">Round</option>
                        </select>
                    </mat-form-field>
                    <button mat-raised-button color="primary" (click)="saveTable()" [disabled]="!newTable.table_number">Save</button>
                </div>
            </mat-card-content>
        </mat-card>
      </div>
      ...
      <div class="floor-plan">
        <div *ngFor="let table of tables" 
            ...
            <!-- Chairs Layout -->
            <div class="chairs-container" [ngClass]="{'round-layout': table.table_type === 'Round', 'square-layout': table.table_type !== 'Round'}">
                <div *ngFor="let chair of getChairs(table.capacity); let i = index" 
                     class="chair"
                     [style.transform]="getChairTransform(i, table.capacity, table.table_type)">
                </div>
            </div>

            <!-- The Table Shape -->
            <div class="table-shape" 
                 [ngClass]="[table.table_type?.toLowerCase() || 'square', table.status.toLowerCase()]"
  ...
  styles: [`
    ...
    .add-form-container { margin-bottom: 20px; }
...
export class TableListComponent implements OnInit {
  ...
  showAddForm = false;
  newTable: { table_number: string, capacity: number | null, type: string } = { table_number: '', capacity: null, type: 'Square' };
  ...
  saveTable() {
    console.log('Saving table:', this.newTable);
    if (this.newTable.table_number && this.newTable.capacity) {
      this.tableService.addTable({
        table_number: this.newTable.table_number,
        capacity: parseInt(this.newTable.capacity.toString(), 10),
        ...
        next: () => {
          ...
          this.newTable = { table_number: '', capacity: null, type: 'Square' };
          this.cdr.detectChanges();
        },
        error: (err) => {
